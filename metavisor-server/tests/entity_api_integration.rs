...
async fn test_create_entity() {
    ...
    assert_eq!(get_body["entity"]["attributes"]["rowCount"], 500);

    // Verify labels
    let labels = get_body["entity"]["labels"]
        .as_array()
        .expect("labels should be an array");
    assert!(labels.contains(&json!("test")), "Should have 'test' label");
    assert!(
        labels.contains(&json!("integration")),
        "Should have 'integration' label"
    );
}
...
#[tokio::test]
async fn test_recreate_entity_after_deletion() {
    let client = client();
    ...
    let guid1 = create_body1["entity"]["guid"]
        .as_str()
        .expect("GUID should be present");
    ...
    let get_response2 = client
        ...
        .await
        .expect("Failed to get entity");
    assert_eq!(get_response2.status(), 404, "Entity should be deleted");

    // 3. Recreate entity (will get a new GUID)
    let create_response2 = client
        ...
        .send()
        .await
        .expect("Failed to recreate entity");
    ...
    let guid2 = create_body2["entity"]["guid"]
        .as_str()
        .expect("GUID should be present");

    // Verify recreation - new GUID should be different
    assert_ne!(guid1, guid2, "New entity should have a different GUID");
...
async fn test_create_entity_with_classification() {
    ...
    let guid = body["entity"]["guid"]
        .as_str()
        .expect("GUID should be present");

    // Verify classifications and labels in create response
    assert_eq!(
    ...
    assert_eq!(get_body["entity"]["guid"], guid);
    assert_eq!(get_body["entity"]["typeName"], "TestTable");
    assert_eq!(get_body["entity"]["attributes"]["name"], "sensitive_table");

    // Verify classifications in get response
    let classifications = get_body["entity"]["classifications"]
        .as_array()
        .expect("classifications should be an array");
    assert_eq!(classifications.len(), 1, "Should have 1 classification");
    assert_eq!(classifications[0]["typeName"], "TestPII");

    // Verify labels in get response
    let labels = get_body["entity"]["labels"]
        .as_array()
        .expect("labels should be an array");
    assert!(
        labels.contains(&json!("sensitive")),
        "Should have 'sensitive' label"
    );
}
